...
class ViewController: UIViewController {
    ...
    func createCanvas() {
        stockView = UIView(frame: CGRect(x: 30, y: 100, width: UIScreen.width - 70, height: UIScreen.width))
        view.addSubview(stockView)
    }
    
    func createShapeLayer() -> CAShapeLayer {
        let shapeLayer = CAShapeLayer()
        shapeLayer.frame = stockView.frame
    ...
    func getAllPoint(_ content: ContentDouble?) {
        ...
        addShapeLayer(shapeLayer, path: path, view: stockView)
